...
export default function CandidatesAdmin(){
  ...
  async function loadCandidates(){
    try{
      setLoading(true);
      const r = await fetch('http://localhost:8080/candidate');
  ...
  async function openCreate(){ setForm({ name:'', party:'', age:''}); setEditing(null); setModalOpen(true); }
  ...
  async function handleSave(e){
    ...
    setSaving(true);
    try{
      const payload = { name: form.name, party: form.party, age: Number(form.age) };
      const url = editing ? `http://localhost:8080/candidate/${editing}` : 'http://localhost:8080/candidate';
      ...
      setToast({ message: editing ? 'Candidate updated' : 'Candidate created', type: 'success' });
      setModalOpen(false);
  ...
  async function handleDelete(id){
    ...
    try{
      const resp = await fetch(`http://localhost:8080/candidate/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
      if(!resp.ok) throw new Error(await resp.text());
    ...
    <div className="cand-admin container fade-in">
      ...
      <header className="cand-header">
        ...
        <div className="header-actions">
          <input className="search" placeholder="Search by name or party" value={search} onChange={e=>setSearch(e.target.value)} />
            ...
            <div key={c._id} className="cand-card">
              ...
              <div className="right">
                ...
                <div className="actions">
                  ...
                  <button className="btn danger" onClick={()=>handleDelete(c._1d || c._id)}>Delete</button>
                </div>
              </div>
            </div>
          ))}
      ...
      <Modal open={modalOpen} title={editing ? 'Edit Candidate' : 'New Candidate'} onClose={()=>setModalOpen(false)}>
