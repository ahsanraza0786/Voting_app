...
router.post("/signup", async (req, res) => {
  try {
    ...
    // Send the response
    return res.status(200).json({ message:"Successfully signup",response: response, token: token });
  } catch (err) {
    console.log(err);
...
router.get('/admin/dashboard', adminAuthMiddleware, async (req, res) => {
  try {
    ...
    // Sum votes across all candidates (assumes candidate.voteCount exists)
    const votesAgg = await Candidate.aggregate([
      { $group: { _id: null, totalVotes: { $sum: '$voteCount' } } }
    ]);
    const totalVotes = votesAgg[0] ? votesAgg[0].totalVotes : 0;

    // Candidate list with minimal fields
    const candidates = await Candidate.find({}, { name: 1, voteCount: 1 }).lean();

    return res.status(200).json({ usersCount, candidatesCount, totalVotes, candidates });
  } catch (err) {
